...
Wanneer een [!DNL Experience] in MFE wordt geselecteerd en de **[!UICONTROL Use]** knoop wordt geklikt, moet een callback functie worden aangehaald om de details van geselecteerde [!DNL Experience] over te gaan.

De callback bevat een HTML-tekenreeks die de [!DNL Experience] vertegenwoordigt. Als de Experience elementen bevat (bijvoorbeeld afbeeldingen, video&#39;s), bevatten HTML-elementen, zoals `<img>` en `<video>` , `src` kenmerken die vooraf ondertekende URL&#39;s voor deze elementen bevatten. Deze vooraf ondertekende URL&#39;s hebben een vervaltijd van 30 minuten en de [!DNL Experience] moet in die tijd door de clienttoepassing worden gebruikt.

### Uitvoerschema

De callback van `onSelectionConfirmed` ontvangt een array van [!DNL Experience] -objecten met de volgende structuur:

| Eigenschap | Type | Beschrijving |
|----------|------|-------------|
| `id` | `string` | Unieke id voor de [!DNL Experience] -notatie (URN-indeling). |
| `content` | `string` | Base64-gecodeerde HTML-tekenreeks die staat voor de [!DNL Experience] . |
| `experienceFields` | `object` | Sleutelwaardeparen van bewerkbare velden in de [!DNL Experience] . Elk veld in `experienceFields` bevat: <ul><li>`fieldName`: De naam/id van het veld.</li><li>`fieldValue`: De werkelijke inhoud/waarde van het veld.</li></ul> |
| `metadata` | `object` | Metagegevens over de ervaring (bijvoorbeeld `channel` , `externalTemplateMetadata` , `externalAssetMetadata` enz.) |
| `aspectVariants` | `array` | Array met varianten van de hoogte-breedteverhouding voor de [!DNL Experience] . Elke variant bevat: <ul><li>`aspectKey`: unieke id voor de variant.</li><li>`aspectMetadata`: Metagegevens, inclusief `channel` en `aspectRatio` .</li><li>`content`: Base64-gecodeerde HTML-tekenreeks voor deze specifieke hoogte-breedteverhouding.</li></ul> |

### Voorbeeld-JSON

Hieronder ziet u een voorbeeld van een JSON-blok met de schema&#39;s.
